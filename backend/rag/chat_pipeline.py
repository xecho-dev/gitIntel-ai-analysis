...
class RAGPipeline:
    ...
    def multi_layer_memory(self) -> Optional[MultiLayerMemory]:
        """懒加载多层记忆"""
        if (
            self._multi_layer_memory is None
            and self.enable_multi_layer_memory
            and create_multi_layer_memory is not None
        ):
            try:
                ...
                _logger.info(
                    f"[RAGPipeline] 多层记忆初始化完成: session={self.session_id}, user_id={self.user_id}"
                )
    ...
    async def chat(
        ...
        yield self._sse_event(
            SSEEventType.ROUTE,
            {
                ...
                "rewrite_enabled": processed_query.rewritten_query
        ...
        if self.multi_layer_memory:
            try:
                ...
                if has_short or has_long or has_profile:
                    ...
                    _logger.debug(
                        "[RAGPipeline] 记忆上下文为空（首次对话或无匹配记忆）"
                    )
            except Exception as exc:
        ...
        has_memory = bool(
            memory_layers.get("short_term")
            or memory_layers.get("long_term")
            or memory_layers.get("profile")
        )
    ...
    async def _retrieve(self, query: ProcessedQuery) -> list:
        ...
        retrieval_query = (
            query.rewritten_query if query.rewritten_query else query.original
        )
