...
async def api_get_history(
    ...
    pool = await get_db()
    return await get_history(pool, auth_user_id, page, page_size, search)
...
async def api_save_analysis(req: SaveAnalysisRequest, request: Request):
    ...
    if not await get_user_uuid(pool, auth_user_id):
        raise HTTPException(
            status_code=404,
    ...
    result = await save_analysis(
        pool, auth_user_id, req.repo_url, req.branch, req.result_data,
...
async def api_delete_history(history_id: str, request: Request):
    ...
    ok = await delete_analysis(pool, auth_user_id, history_id)
    if not ok:
