...
class FixGeneratorAgent:
    ...
    async def stream(
        ...
        """
        yield _make_event(
            self.name, "status", "正在分析优化建议，生成代码修改方案…", 10, None
        )
        ...
        yield _make_event(
            self.name, "progress", f"正在生成 {len(actionable)} 个代码修改…", 30, None
        )
        ...
        if not llm:
            yield _make_event(
                self.name, "error", "未配置 LLM API，无法生成代码修改", 0, None
            )
            return

        try:
            fixes = await self._generate_fixes(
                llm, repo_path, branch, actionable, file_contents
            )
            ...
            yield _make_event(self.name, "error", f"生成代码修改失败: {exc}", 0, None)
...
直接返回 JSON 数组（最多 {len(need_llm)} 个修改）："""
        ...
        try:
            ...
            _logger.warning(
                f"[FixGeneratorAgent] LLM 生成失败，降级保留已有 fixes: {exc}"
            )
